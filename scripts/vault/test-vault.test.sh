...
git -C "$FIXTURE_ROOT" add .gitignore manifest.json scripts/vault/test-vault.sh source.txt
...
run_deploy() {
  ...
  PATH="$FAKE_BIN:$PATH" \
    ...
    bash "$FIXTURE_ROOT/scripts/vault/test-vault.sh" >/dev/null 2>"$DEPLOY_STDERR"
}
