import { OBSIDIAN_PUBLISH_BASELINE } from "@/open-artifacts/obsidian-publish-baseline";
import { OPENARTIFACTS_MAX_HTML_BYTES } from "@/open-artifacts/constants";
import { OpenArtifactsDocument } from "@/open-artifacts/types";
...
export class OpenArtifactsDocumentTooLargeError extends Error {
  ...
  constructor(public readonly byteLength: number) {
    ...
    this.name = "OpenArtifactsDocumentTooLargeError";
    Object.setPrototypeOf(this, OpenArtifactsDocumentTooLargeError.prototype);
  }
}
...
export async function buildOpenArtifactsDocument(
  ...
  const article = ownerDocument.win.createEl(
    "article",
    "markdown-preview-view markdown-rendered openartifacts-document"
  );
  ...
  if (projectedByteLength > OPENARTIFACTS_MAX_HTML_BYTES) {
    throw new OpenArtifactsDocumentTooLargeError(projectedByteLength);
  }
  ...
  return createOpenArtifactsDocument(title, html);
}
...
async function embedVaultImages(
  ...
  for (const image of images) {
    ...
    if (embeddedDocumentByteLength > OPENARTIFACTS_MAX_HTML_BYTES) {
      throw new OpenArtifactsDocumentTooLargeError(embeddedDocumentByteLength);
    }
...
function replaceMissingImage(image: HTMLImageElement, source: string): HTMLElement {
  const replacement = image.doc.win.createSpan({
    cls: "openartifacts-missing-asset",
...
function serializeDocument(ownerDocument: Document, title: string, article: HTMLElement): string {
  ...
  return [
    ...
    `<style id="openartifacts-obsidian-publish-baseline">${OBSIDIAN_PUBLISH_BASELINE}</style>`,
    "</head>",
