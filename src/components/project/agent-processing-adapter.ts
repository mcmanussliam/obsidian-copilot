...
import type { AgentProjectContextLoadState, FailedItem } from "@/ai-params";
import { EMPTY_PROCESSING_SOURCES, EMPTY_RETRYING_SOURCES } from "@/ai-params";
import {
  processingItemEnvelope,
  type ProcessingItem,
} from "@/components/project/processing-adapter";
import {
  cacheFileName,
  failureMarkerName,
  parseFailureMarker,
...
import type { UrlItem } from "@/utils/url-tag-utils";
...
export async function readAgentCacheDirState(
  ...
  try {
    const { remotesDir, filesDir, markersDir } = await import("@/context/conversions-location");
