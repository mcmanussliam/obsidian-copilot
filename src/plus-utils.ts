import { setModelKey } from "@/ai-params";
import { CopilotPlusExpiredModal } from "@/components/modals/copilot-plus-expired-modal";
import { ChatModelProviders, ChatModels } from "@/constants";
import { EntitlementFeature, verifyEntitlement } from "@/entitlement";
import { BrevilabsClient, LicenseCheckContext } from "@/llm-providers/brevilabs-client";
import { createProductUrl, PRODUCT_URLS, ProductUtmMedium } from "@/lib/product-links";
...
export async function applyLicenseSettings(): Promise<void> {
  ...
  try {
    const { applyCopilotDefaultModel } = await import("@/agent-mode");
