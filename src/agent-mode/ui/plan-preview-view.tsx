...
import type { CurrentPlan, PlanDecisionAction } from "@/agent-mode/session/types";
 ...
 * (Claude re-issuing ExitPlanMode, OpenCode emitting a new structured plan)
 * swap the rendered markdown in place. When `currentPlan` becomes `null`
 * the view shows an empty "Plan no longer pending" state instead of a stale
 * snapshot.
 */
 ...
 * `src/components/composer/apply-view.tsx`).
 *
...
/**
 * Plugin reloads can leave Obsidian holding a stale view object for this
 * custom view type. Treat those as non-matching instead of assuming the
 * current class methods exist.
 */
