...
import { DebuggingSupportSection } from "@/settings/v2/components/debugging-support-section";
import { LegacyChatPromptsNotice } from "@/settings/v2/components/legacy-chat-prompts-notice";
import { useApp } from "@/context";
import { getCopilotSaveData } from "@/settings/copilot-save-data";
...
import { hasPersistedSecrets } from "@/services/settings-secret-transforms";
...
import { getPersistedDeviceId } from "@/utils/device-id";
import { createReportUploader } from "@/utils/report-upload.brevilabs";
...
export const AdvancedSettings: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void import("@/agent-mode").then(({ acpFrameSink }) => {
      if (!cancelled) {
  ...
  const handleReportIssue = useCallback(() => {
    ...
    void (async () => {
      const { ReportIssueModal } = await import("@/agent-mode");
      const copilotPlugin = (
        app as unknown as {
          plugins: {
            getPlugin: (id: string) => {
  ...
  const handleOpenFrameLog = useCallback(async () => {
    ...
    try {
      const { acpFrameSink } = await import("@/agent-mode");
