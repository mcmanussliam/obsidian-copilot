...
import { openAgentsFile, writeAgentsFile } from "@/instructions/agents-file";
import { useAgentsFileDraft } from "@/instructions/use-agents-file-draft";
import { logError } from "@/logger";
import { debounce } from "@/utils/debounce";
import { ensureCopilotSubfolders } from "@/settings/copilot-folder";
import {
  applyCopilotRootChange,
  copilotRootContainsNotes,
  findCopilotRootFileConflict,
} from "@/settings/copilot-root-change";
import { updateSetting, useSettingsValue, validateCopilotFolder } from "@/settings/model";
import { QuickChatPanel } from "@/settings/v2/components/quick-chat-panel";
import { DesktopOnlySettingsPanel } from "@/settings/v2/components/desktop-only-settings-panel";
import { CopilotFolderChangeNotice } from "@/settings/v2/components/copilot-folder-change-notice";
import { LegacyChatPromptsNotice } from "@/settings/v2/components/legacy-chat-prompts-notice";
import { PlusSettings } from "@/settings/v2/components/plus-settings";
import { VaultInstructionsSetting } from "@/settings/v2/components/vault-instructions-setting";
...
const LazyAgentSettings = React.lazy(() =>
  import("@/settings/v2/components/agent-settings").then((module) => ({
 ...
 * the `@/agent-mode` barrel, whose Node-only dependencies cannot run on mobile.
 */
