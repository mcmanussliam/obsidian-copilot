import { ResetSettingsConfirmModal } from "@/components/modals/reset-settings-confirm-modal";
...
import { ByokPanel, ModelManagementProvider } from "@/model-management";
import { resetSettings } from "@/settings/model";
import { COPILOT_SETTINGS_TAB_IDS, type CopilotSettingsTabId } from "@/settings/settings-tabs";
import { useSkillLoadErrorCount } from "@/settings/skill-load-error-state";
import { CommandSettings } from "@/settings/v2/components/command-settings";
...
import { DesktopOnlySettingsPanel } from "./components/desktop-only-settings-panel";
import { MiyoSettings } from "./components/miyo-settings";
import { SelfHostSettings } from "./components/self-host-settings";
...
const LazySkillsSettings = React.lazy(() =>
  import("@/agent-mode").then((module) => ({ default: module.SkillsSettings }))
);

const SkillsSettingsPanel: React.FC = () => {
  // Gate before the dynamic import: on mobile the `@/agent-mode` barrel pulls in
