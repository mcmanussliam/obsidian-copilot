...
import { useAgentInputDrafts } from "@/agent-mode/ui/hooks/use-agent-input-drafts";
...
jest.mock("@/agent-mode/ui/mentioned-agents", () => ({
...
jest.mock("@/components/chat-components/chat-input", () => ({
  __esModule: true,
  default: jest.requireActual<typeof React>("react").forwardRef(
    (
      ...
      ref: React.ForwardedRef<import("@/components/chat-components/chat-input").ChatInputHandle>
    ) => {
...
jest.mock("@/agent-mode/session/expand-custom-command-prefix", () => ({
  expandCustomCommandPrefix: jest.fn(async (text: string) => ({ text })),
}));
jest.mock("@/services/web-viewer-service/active-web-tab-snapshot", () => ({
...
describe("AgentChatInput", () => {
  ...
  describe("identity and agent-mention gate", () => {
    ...
    it("clears input-scoped context only when the logical chat input changes", () => {
      const clearSelectedTextContexts = jest.requireMock("@/ai-params")
