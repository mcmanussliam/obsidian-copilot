...
jest.mock("@/projects/project-paths", () => ({
...
jest.mock("@/components/modals/project/add-project-modal", () => ({
  AddProjectModal: jest.fn().mockImplementation(() => ({ open: jest.fn() })),
}));
const revealProjectFolder = jest.fn();
jest.mock("@/agent-mode/ui/agent-project-row-actions", () => ({
...
jest.mock("@/instructions/agents-file", () => ({
  ...
  isClaudeImportOnly: jest.requireActual("@/instructions/agents-file").isClaudeImportOnly,
}));
const moveProjectPromptToAgentsFile = jest.fn().mockResolvedValue(undefined);
jest.mock("@/projects/move-project-prompt", () => ({
...
import type { AgentTodoListEntry } from "@/agent-mode/session/types";
import type { ProjectConfig } from "@/ai-params";
