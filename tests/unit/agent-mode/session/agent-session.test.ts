...
import { OpencodeBackendDescriptor } from "@/agent-mode/backends/opencode/descriptor";
import { AI_SENDER, USER_SENDER } from "@/constants";
import { ClaudeBackendDescriptor } from "@/agent-mode/backends/claude/descriptor";
...
} from "@/agent-mode/session/agent-session";
import { ensureMultiAgentEntitlement, showMultiAgentUpgradePrompt } from "@/plus-utils";
...
import type { ApplySelectionContext } from "@/agent-mode/session/descriptor";
import type { FanoutRunInput } from "@/agent-mode/session/fanout/fanout-orchestrator";
import {
  FANOUT_READONLY_PREAMBLE,
  type FanoutTurn,
} from "@/agent-mode/session/fanout/fanout-types";
import type {
  AgentToolCallOutput,
  BackendDescriptor,
  BackendProcess,
...
describe("ensureMultiAgentEntitlement (paywall helper)", () => {
  ...
  async function loadHelper(isPlus: boolean): Promise<(app?: unknown) => Promise<boolean>> {
    ...
    jest.doMock("@/plus-utils", () => jest.requireActual("@/plus-utils"));
    jest.doMock("@/logger", () => ({
      logInfo: jest.fn(),
      logWarn: jest.fn(),
      logError: jest.fn(),
    ...
    jest.doMock("@/llm-providers/brevilabs-client", () => ({
      BrevilabsClient: { getInstance: () => ({ validateLicenseKey }) },
    }));
    const mod = await import("@/plus-utils");
