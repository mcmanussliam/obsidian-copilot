import { composeDenyList } from "@/agent-mode/skills/deny-list-composer";
import { BUILTIN_SKILLS, RETIRED_BUILTIN_SKILLS } from "@/builtin-skills/builtin-skills";
import { sanitizeBuiltinPreferences } from "@/settings/model";
import type { BuiltinPreferences } from "@/agent-mode/skills/builtin/reconcile-builtin-skills";
import { act, renderHook } from "@testing-library/react";
import { FileSystemAdapter, type App, type EventRef } from "obsidian";
import { useSkillLoadErrorCount } from "@/settings/skill-load-error-state";
import { discoverManagedSkills } from "@/agent-mode/skills/discover-managed-skills";
import { reconcile } from "@/agent-mode/skills/reconcile";
import {
  ...
  type RefreshResult,
  useRejectedSkills,
} from "@/agent-mode/skills/skill-manager";
import { runDeleteSkill, runToggleAgent } from "@/agent-mode/skills/toggle-agent";
import { runRenameSkill, runUpdateProperties } from "@/agent-mode/skills/update-properties";
import type { RejectedSkill, Skill, SkillDiscoveryResult } from "@/agent-mode/skills/types";
...
jest.mock("@/settings/copilot-folder", () => ({
  getEffectiveSkillsFolder: () => skillsFolder,
}));

jest.mock("@/agent-mode/skills/discover-managed-skills", () => ({
  discoverManagedSkills: jest.fn(),
}));

jest.mock("@/agent-mode/skills/reconcile", () => ({
  reconcile: jest.fn(async () => ({ created: [], removedOrphans: [], errors: [] })),
}));

jest.mock("@/agent-mode/skills/node-fs-adapters", () => ({
  createNodeMigrateSkillFs: jest.fn(() => ({})),
  createNodeProjectDiscoveryFs: jest.fn(() => ({})),
  createNodeReconcileFs: jest.fn(() => ({})),
}));

jest.mock("@/agent-mode/skills/discover-project-skills", () => ({
  discoverProjectSkills: jest.fn(async () => ({ accepted: [], rejected: [] })),
}));

jest.mock("@/agent-mode/skills/merge-discovery", () => {
  const actual = jest.requireActual("@/agent-mode/skills/merge-discovery");
  return {
    ...actual,
...
jest.mock("@/agent-mode/skills/toggle-agent", () => ({
  runDeleteSkill: jest.fn(),
  runToggleAgent: jest.fn(),
}));

jest.mock("@/agent-mode/skills/update-properties", () => ({
