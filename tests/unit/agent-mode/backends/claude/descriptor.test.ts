import { signOutFromClaude } from "@/agent-mode/backends/claude/claude-auth";
jest.mock("@/agent-mode/backends/claude/claude-auth", () => ({ signOutFromClaude: jest.fn() }));

import type { AgentSession } from "@/agent-mode/session/agent-session";
import type { BackendState, InstallState } from "@/agent-mode/session/types";
import { resetSettings, setSettings, type CopilotSettings } from "@/settings/model";
import { MIYO_SEARCH_FOLDER_ENV, MIYO_SEARCH_SCOPE_ENV } from "@/builtin-skills/builtin-skills";
import { __resetVaultBaseCache } from "@/utils/vault-path";
import { FileSystemAdapter, type App } from "obsidian";
import { resolveClaudeBinary } from "@/agent-mode/backends/claude/claude-binary-resolver";
import { claudeCompatibilityStore } from "@/agent-mode/backends/claude/claude-compatibility-store";
import {
  ...
  updateClaudeFields,
} from "@/agent-mode/backends/claude/descriptor";

jest.mock("@/agent-mode/backends/claude/claude-binary-resolver", () => ({
  claudeBinarySearchDirs: jest.fn(() => []),
  resolveClaudeBinary: jest.fn(),
}));

jest.mock("@/agent-mode/backends/claude/claude-compatibility-store", () => ({
