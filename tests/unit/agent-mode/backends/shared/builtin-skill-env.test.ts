...
} from "@/agent-mode/backends/shared/obsidian-cli-path";

jest.mock("@/settings/model", () => ({ getSettings: jest.fn() }));
jest.mock("@/miyo/miyo-utils", () => ({ getMiyoCustomUrl: jest.fn() }));
jest.mock("@/logger", () => ({ logWarn: jest.fn() }));
jest.mock("@/agent-mode/backends/shared/obsidian-cli-path", () => ({
