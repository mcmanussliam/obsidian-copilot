...
import type { InstallState } from "@/agent-mode/session/types";
import type { BuiltinSkillRuntime } from "@/agent-mode/skills/skill-manager";
import { createAgentSessionManager } from "@/agent-mode";
import { reconcileBuiltinSkills } from "@/agent-mode/skills/builtin/reconcile-builtin-skills";
import { BackendConfigRegistry } from "@/model-management";
...
jest.mock("@/model-management", () => ({
  BackendConfigRegistry: jest.requireActual("@/model-management/backends/backend-config-registry")
...
jest.mock("@/settings/copilot-folder", () => ({ deriveSkillsFolder: () => "copilot/skills" }));
jest.mock("@/settings/builtin-skill-preferences", () => ({ saveBuiltinPreferences: jest.fn() }));
jest.mock("@/system-prompts/state", () => ({ subscribeToSystemPromptChange: jest.fn() }));
jest.mock("@/utils/app-paths", () => ({
  copilotAppDataDir: () => "/tmp/copilot",
  getVaultId: () => "test-vault",
}));
jest.mock("@/agent-mode/backends/shared/agent-system-prompt", () => ({
  buildAgentSystemPrompt: () => "prompt",
}));
jest.mock("@/agent-mode/backends/shared/builtin-skill-env", () => ({
  getBuiltinSkillEnvRestartPolicy: () => "none",
}));
jest.mock("@/agent-mode/backends/registry", () => ({
  backendRegistry: {},
  listBackendDescriptors: () => mockDescriptors,
}));
jest.mock("@/agent-mode/session/agent-chat-persistence-manager", () => ({
  AgentChatPersistenceManager: jest.fn(),
}));
jest.mock("@/agent-mode/session/agent-model-preloader", () => ({ AgentModelPreloader: jest.fn() }));
jest.mock("@/agent-mode/session/agent-session-index", () => ({ AgentSessionIndex: jest.fn() }));
jest.mock("@/agent-mode/session/node-file-storage", () => ({ createNodeFileStorage: jest.fn() }));
jest.mock("@/agent-mode/session/agent-session-manager", () => ({
  AgentSessionManager: jest.fn(() => mockManager),
}));
jest.mock("@/agent-mode/session/copilot-default-model", () => ({
  seedCopilotDefaultModel: jest.fn(),
}));
jest.mock("@/agent-mode/skills", () => ({
  SkillManager: {
...
jest.mock("@/agent-mode/skills/builtin/reconcile-builtin-skills", () => ({
  ...jest.requireActual("@/agent-mode/skills/builtin/reconcile-builtin-skills"),
  reconcileBuiltinSkills: jest.fn(),
}));
jest.mock("@/agent-mode/skills/builtin/miyo-search-seed", () => ({
  buildBuiltinSeedFs: jest.fn(),
}));
jest.mock("@/agent-mode/ui/permission-prompter", () => ({
...
jest.mock("@/agent-mode/ui/agent-mode-chat", () => ({}));
jest.mock("@/agent-mode/ui/copilot-agent-view", () => ({}));
jest.mock("@/agent-mode/ui/use-backend-descriptor", () => ({}));
jest.mock("@/agent-mode/ui/use-agent-model-picker", () => ({}));
jest.mock("@/agent-mode/ui/use-agent-mode-picker", () => ({}));
jest.mock("@/agent-mode/backends/opencode/opencode-probe-partition", () => ({}));
jest.mock("@/agent-mode/backends/opencode/opencode-model-resolve", () => ({}));
jest.mock("@/agent-mode/backends/shared/install-status", () => ({}));
jest.mock("@/agent-mode/ui/agent-default-model-setting", () => ({}));
jest.mock("@/components/ui/model-enable-list", () => ({}));
jest.mock("@/agent-mode/ui/plan-preview-view", () => ({}));
jest.mock("@/agent-mode/ui/report-issue-modal", () => ({}));
jest.mock("@/agent-mode/session/debug-sink", () => ({}));
jest.mock("@/agent-mode/backends/shared/ui/agent-backend-header", () => ({}));
jest.mock("@/agent-mode/session/use-backend-auth-state", () => ({}));
