import { composeDenyList } from "@/agentMode/skills/denyListComposer";
...
import type { BuiltinPreferences } from "@/agentMode/skills/builtin/reconcileBuiltinSkills";
...
import { discoverManagedSkills } from "@/agentMode/skills/discoverManagedSkills";
import { reconcile } from "@/agentMode/skills/reconcile";
import {
...
import { runDeleteSkill, runToggleAgent } from "@/agentMode/skills/toggleAgent";
import { runRenameSkill, runUpdateProperties } from "@/agentMode/skills/updateProperties";
import type { RejectedSkill, Skill, SkillDiscoveryResult } from "@/agentMode/skills/types";
...
jest.mock("@/agentMode/skills/discoverManagedSkills", () => ({
  discoverManagedSkills: jest.fn(),
}));

jest.mock("@/agentMode/skills/reconcile", () => ({
  reconcile: jest.fn(async () => ({ created: [], removedOrphans: [], errors: [] })),
}));

jest.mock("@/agentMode/skills/nodeFsAdapters", () => ({
  createNodeMigrateSkillFs: jest.fn(() => ({})),
  createNodeProjectDiscoveryFs: jest.fn(() => ({})),
  createNodeReconcileFs: jest.fn(() => ({})),
}));

jest.mock("@/agentMode/skills/discoverProjectSkills", () => ({
  discoverProjectSkills: jest.fn(async () => ({ accepted: [], rejected: [] })),
}));

jest.mock("@/agentMode/skills/mergeDiscovery", () => {
  const actual = jest.requireActual("@/agentMode/skills/mergeDiscovery");
  return {
    ...actual,
...
jest.mock("@/agentMode/skills/toggleAgent", () => ({
  runDeleteSkill: jest.fn(),
  runToggleAgent: jest.fn(),
}));

jest.mock("@/agentMode/skills/updateProperties", () => ({
