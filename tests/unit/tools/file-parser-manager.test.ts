...
jest.mock("@/miyo/miyo-runtime-policy", () => ({
...
jest.mock("@/cache/pdf-cache", () => ({
  PDFCache: {
    getInstance: () => ({ get: mockPdfCacheGet, set: mockPdfCacheSet }),
  },
}));

jest.mock("@/utils/converted-doc-output", () => ({
...
import type { BrevilabsClient } from "@/llm-providers/brevilabs-client";
import { isMiyoAvailableForCapability } from "@/miyo/miyo-status-store";
import type { CopilotSettings } from "@/settings/model";
import type { TFile, Vault } from "obsidian";
import { Docs4LLMParser, PDFParser } from "@/tools/file-parser-manager";
