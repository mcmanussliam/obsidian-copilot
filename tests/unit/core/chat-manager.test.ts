...
jest.mock("@/core/message-repository");
jest.mock("@/llm-providers/chat-model-manager");
jest.mock("@/core/context-compactor");
...
jest.mock("@/core/chat-persistence-manager", () => ({
...
jest.mock("@/system-prompts/system-prompt-builder", () => ({
...
import { ChatPersistenceManager } from "@/core/chat-persistence-manager";
import { ChatManager } from "@/core/chat-manager";
import { MessageRepository } from "@/core/message-repository";
import { ContextManager } from "@/core/context-manager";
import type ChainManager from "@/llm-providers/chain-manager";
import type { FileParserManager } from "@/tools/file-parser-manager";
import type CopilotPlugin from "@/main";
import { ChainType } from "@/chain-type";
import { getWebViewerService } from "@/services/web-viewer-service/web-viewer-service-singleton";
...
describe("ChatManager", () => {
  ...
  describe("vault instruction integration", () => {
    const builder = jest.requireMock<Record<string, jest.Mock>>(
      "@/system-prompts/system-prompt-builder"
    );
    const actualBuilder = jest.requireActual<
      typeof import("@/system-prompts/system-prompt-builder")
    >("@/system-prompts/system-prompt-builder");
    const state =
    ...
    describe("sendMessage()", () => {
      ...
      it("uses one instruction snapshot through memory loading and template expansion (https://github.com/logancyang/obsidian-copilot/issues/3210)", async () => {
        ...
        jest.requireMock("@/commands/custom-command-utils").processPrompt.mockResolvedValueOnce({
  ...
  describe("truncateAfterMessageId", () => {
    it("should truncate messages and update chain memory", async () => {
      const { updateChatMemory } = await import("@/chat-utils");
