...
import { detectBinary, validateExecutableFile } from "@/utils/detect-binary";
...
jest.mock("@/utils/node-tool-bin-dirs", () => ({ resolveNodeToolBinDirs: jest.fn(() => []) }));
...
describe("detectBinary", () => {
  describe("detectBinary()", () => {
    ...
    test("posix: augments PATH with /opt/homebrew/bin and /usr/local/bin ahead of inherited PATH", async () => {
      ...
      jest.isolateModules(() => {
        detectPosixBinary =
          jest.requireActual<typeof import("@/utils/detect-binary")>(
            "@/utils/detect-binary"
          ).detectBinary;
      });
    ...
    test("does not require Node built-ins at module evaluation time", () => {
      ...
      try {
        jest.isolateModules(() => {
          ...
          expect(() => void jest.requireActual("@/utils/detect-binary")).not.toThrow();
        });
      } finally {
