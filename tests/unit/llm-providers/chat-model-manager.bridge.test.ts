...
import ChatModelManager from "@/llm-providers/chat-model-manager";
...
jest.mock("@/llm-providers/chat-open-router", () => {
  ...
  describe("ChatModelManager", () => {
    ...
    describe("createModelInstanceFromBridged()", () => {
      ...
      it("sends the plugin version with Copilot Plus chat requests", async () => {
        const OpenRouterMock = jest.requireMock("@/llm-providers/chat-open-router")
          .ChatOpenRouter as {
