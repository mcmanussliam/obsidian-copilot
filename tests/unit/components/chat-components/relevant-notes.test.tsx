...
import { getChatRelevantNotesStore } from "@/search/chat-relevant-notes-context";
import { findChatRelevantNotes } from "@/search/find-chat-relevant-notes";
import { RelevantNotes } from "@/components/chat-components/relevant-notes";
import { useActiveFile } from "@/hooks/use-active-file";
import { LIVE_REFRESH_INTERVAL_MS } from "@/hooks/use-live-relevant-notes-refresh";
import { findRelevantNotes } from "@/search/find-relevant-notes";
...
jest.mock("@/hooks/use-active-file", () => ({
  useActiveFile: jest.fn(),
}));

jest.mock("@/hooks/use-note-drag", () => ({
  useNoteDrag: () => jest.fn(),
}));

jest.mock("@/miyo/use-miyo-status", () => ({
  useMiyoStatus: () => ({ backend: mockMiyoBackend }),
}));

jest.mock("@/search/find-relevant-notes", () => ({
  ...jest.requireActual<typeof import("@/search/find-relevant-notes")>(
    "@/search/find-relevant-notes"
  ),
  findRelevantNotes: jest.fn(),
}));

jest.mock("@/search/find-chat-relevant-notes", () => ({ findChatRelevantNotes: jest.fn() }));
...
jest.mock("@/settings/open-settings", () => ({ openCopilotSettings: jest.fn() }));
