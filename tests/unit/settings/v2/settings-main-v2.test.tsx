...
jest.mock("@/settings/v2/components/basic-settings", () => ({ BasicSettings: () => null }));
jest.mock("@/settings/v2/components/miyo-settings", () => ({
  MiyoSettings: () => <div>Miyo settings content</div>,
}));
jest.mock("@/settings/v2/components/self-host-settings", () => ({ SelfHostSettings: () => null }));
jest.mock("@/settings/v2/components/command-settings", () => ({ CommandSettings: () => null }));
jest.mock("@/settings/v2/components/advanced-settings", () => ({ AdvancedSettings: () => null }));
jest.mock("@/settings/v2/components/desktop-only-settings-panel", () => ({
...
jest.mock("@/settings/skill-load-error-state", () => ({
...
jest.mock("@/hooks/use-latest-version", () => ({
  // eslint-disable-next-line @eslint-react/hooks-extra/no-unnecessary-use-prefix -- mocks the real hook; the name must match the export
  useLatestVersion: () => ({ latestVersion: mockLatestVersion, hasUpdate: mockHasUpdate }),
}));
jest.mock("@/utils/desktop-runtime", () => ({ isDesktopRuntime: () => true }));
