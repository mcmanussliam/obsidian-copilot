...
jest.mock("@/miyo/miyo-status-store", () => ({
...
jest.mock("@/miyo/miyo-utils", () => ({
  getMiyoCustomUrl: jest.requireActual<typeof import("@/miyo/miyo-runtime-policy")>(
    "@/miyo/miyo-runtime-policy"
  ).getMiyoCustomUrl,
...
jest.mock("@/settings/v2/components/miyo-connect-modal", () => ({
...
jest.mock("@/utils/vault-path", () => ({ getVaultBase: () => "/vault" }));
...
jest.mock("@/utils/desktop-runtime", () => ({ isDesktopRuntime: () => mockOnDesktop() }));
...
import { MiyoSettings } from "@/settings/v2/components/miyo-settings";
import { refreshMiyoStatus } from "@/miyo/miyo-status-store";
