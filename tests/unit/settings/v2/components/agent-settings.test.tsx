import type { BackendAuthStatus } from "@/agent-mode/session/types";
...
import { AgentSettings } from "@/settings/v2/components/agent-settings";

jest.mock("@/logger", () => ({ logInfo: jest.fn(), logWarn: jest.fn(), logError: jest.fn() }));

jest.mock("@/utils/notification-sound", () => {
  const actual = jest.requireActual<object>("@/utils/notification-sound");
...
// Mock the chat-backend options hook to avoid pulling the heavy @/model-management
// dependency chain (ByokPanel -> ConfirmModal extends Modal) into the test.
jest.mock("@/hooks/use-chat-backend-model-options", () => ({
  // eslint-disable-next-line @eslint-react/hooks-extra/no-unnecessary-use-prefix -- mocks the real hook; name must match the export
...
jest.mock("@/agent-mode", () => ({
  ...
  AgentBackendHeader: jest.requireActual<
    typeof import("@/agent-mode/backends/shared/ui/agent-backend-header")
  >("@/agent-mode/backends/shared/ui/agent-backend-header").AgentBackendHeader,
  ...
  InstallBadge: jest.requireActual<typeof import("@/agent-mode/backends/shared/install-status")>(
    "@/agent-mode/backends/shared/install-status"
  ).InstallBadge,
...
jest.mock("@/settings/v2/components/chat-model-enable-list", () => ({
  ChatModelEnableList: () => <div data-testid="chat-model-list">chat models</div>,
}));

jest.mock("@/settings/v2/components/configured-model-enable-list", () => ({
