...
import { BasicSettings } from "@/settings/v2/components/basic-settings";
import { act, fireEvent, render, screen, waitFor } from "@testing-library/react";
import { Notice } from "obsidian";
import React from "react";

// Stub the Plus banner to keep its dependency chain out of the test.
jest.mock("@/settings/v2/components/plus-settings", () => ({ PlusSettings: () => null }));
...
jest.mock("@/settings/v2/components/agent-settings", () => ({
  AgentSettings: () => <div data-testid="agents-section">agents</div>,
}));

jest.mock("@/settings/v2/components/quick-chat-panel", () => ({
  QuickChatPanel: () => <div>Quick Chat models</div>,
}));

const isDesktopRuntime = jest.fn<boolean, []>().mockReturnValue(true);
jest.mock("@/utils/desktop-runtime", () => ({ isDesktopRuntime: () => isDesktopRuntime() }));
...
jest.mock("@/settings/copilot-root-change", () => ({
...
jest.mock("@/utils/vault-path", () => ({ getVaultBase: () => "/abs/vault" }));
...
jest.mock("@/components/modals/confirm-modal", () => ({
  ConfirmModal: class {
    open = jest.fn();
    constructor(
      _app: unknown,
